...
import { setLogout, setMode } from "@store/slices/globalSlice";
	...
	<FlexBetween padding="1rem 6%" backgroundColor={alt}>
		...
		{isNonMobileScreens ? (
			<FlexBetween gap="2rem">
				<IconButton onClick={() => dispatch(setMode())}>{isDarkMode ? <DarkMode sx={{ fontSize: "25px" }} /> : <LightMode sx={{ color: dark, fontSize: "25px" }} />}</IconButton>
			...
			<Box position="fixed" right="0" bottom="0" height="100%" zIndex="10" maxWidth="500px" minWidth="300px" backgroundColor={background}>
				...
				<FlexBetween display="flex" flexDirection="column" justifyContent="center" alignItems="center" gap="3rem">
					<IconButton onClick={() => dispatch(setMode())} sx={{ fontSize: "25px" }}>
						{isDarkMode ? <DarkMode sx={{ fontSize: "25px" }} /> : <LightMode sx={{ color: dark, fontSize: "25px" }} />}
					</IconButton>
					<Message sx={{ color: isDarkMode ? "white" : "dark", fontSize: "25px" }} />
					<Notifications sx={{ color: isDarkMode ? "white" : "dark", fontSize: "25px" }} />
					<Help sx={{ color: isDarkMode ? "white" : "dark", fontSize: "25px" }} />
